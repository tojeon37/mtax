...
import { getClients, createClient, updateClient } from '../api/clientApi'

const ClientEditPage: React.FC = () => {
  const navigate = useNavigate()
  const location = useLocation()
  const { id } = useParams<{ id: string }>()
  // /clients/new 경로이거나 id가 'new'인 경우 새로 등록
  const isNew = location.pathname === '/clients/new' || id === 'new'
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isNew) {
        ...
        // 기존 거래처 수정
        await updateClient(parseInt(id), submitData)
    ...
    <div className="px-4 py-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">
          {isNew ? '새 거래처 등록' : '거래처 정보 수정'}
        </h1>
        <button
          onClick={() => navigate('/clients')}
          className="text-gray-400 dark:text-gray-300 text-xl hover:text-gray-600 dark:hover:text-gray-200"
        >
          ×
        </button>
      </div>
